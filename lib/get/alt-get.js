...
function getPath(
   ...
   // array with the result of evaluating the rest of path on the target.
      ...
      //debugger;
      let hasValue = curNode.hasValue
      
      let missingChildRelPaths = hasValue ? undefined : missingRelPaths

      for (let refTargetIndex = 0, len = curNode.length; refTargetIndex < len; refTargetIndex++) {

         let childNode = curNode[refTargetIndex]
      ...
      if (hasValue && missingRelPaths) {
         let restOfKeys = pathSet.slice(pathSetIndex);
         relPathSoFar.push.apply(relPathSoFar, restOfKeys);
         missingRelPaths[missingRelPaths.length] = relPathSoFar;
      }
...
getRefTarget = 
   ...
   while(true) {
      ...
      if (curNode === undefined) {
         //debugger;
         let restOfAbsKeys = pathSet.slice(pathSetIndex)
         absPathSoFar.push.apply(absPathSoFar, restOfAbsKeys);

         let restOfRelKeys = relPathSet.slice(relPathSetIndex)
      ...
      if (key == null) {
         ...
         // return getRefTarget(root, curNode[key], pathSet, pathSetIndex + 1, absPathSoFar, relPathSoFar, missingAbsPaths);
...
function get$(model, pathSets) {
   let missingAbsPaths = [];
   let missingRelPaths = [];
   let cache = model._root.cache
   let materialize = model._materialized && !model._source;
   let treatErrorsAsValues = model._treatErrorsAsValues;
   let result = {
      values: [{
         json: getPath(cache, cache, pathSets[0], 0, [], [], missingAbsPaths, missingRelPaths, materialize, treatErrorsAsValues),
      }],
      optimizedMissingPaths: missingAbsPaths, //absolute
      requestedMissingPaths: missingRelPaths  //reletive      
   }
